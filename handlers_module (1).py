...
def register_handlers(user_client, bot_client):
    ...
    async def pdf_image_callback(event):
        ...
        await event.edit(
            ...
            "3. Bot will find & remove matching pages\n\n"
            "**Tips:**\n"
            "• Screenshot the full page for best results\n"
            "• Works even if page has slight differences\n"
            "• Uses AI perceptual hashing\n\n"
    ...
    async def message_handler(event):
        ...
        if step == 'fname_find':
            ...
            try:
                ...
                session['settings']['pdf_image_threshold'] = 10  # Default threshold
                session['step'] = 'settings'
                
                await event.respond(
                    "✅ **Screenshot saved!**\n\n"
                    "📸 Image will be used to find matching pages\n"
                    "🎯 Similarity threshold: **10** (adjustable)\n\n"
                    "Matching pages will be removed during transfer.",
                    buttons=get_settings_keyboard(session_id)
                )
