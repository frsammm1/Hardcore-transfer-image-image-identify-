        ...
        async for message in user_client.iter_messages(
            ...
            while retries > 0 and not success:
                try:
                    ...
                    if file_name.lower().endswith('.pdf') and (settings.get('pdf_pages_list') or settings.get('pdf_keywords')):
                        try:
                            ...
                            # Remove by image matching
                            if settings.get('pdf_reference_image'):
                                ref_image_path = settings['pdf_reference_image']
                                threshold = settings.get('pdf_image_threshold', 10)
                                
                                config.logger.info(f"📸 Searching for matching pages using image...")
                                ...
                                config.logger.info(f"📸 Pages by image: {image_matched_pages}")
                            ...
                            if pages_to_remove:
                                ...
                                if temp_pdf_path:
                                    ...
                                    # Use modified PDF for upload
                                    media_obj = temp_pdf_path
                                    
                                    await status_message.edit(
                                        f"📄 **PDF Processed**\n"
                                        f"Kept: {kept} pages\n"
                                        f"Removed: {removed} pages\n"
                                        f"Uploading...",
                                        buttons=get_progress_keyboard()
                                    )
