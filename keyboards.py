...
def get_settings_keyboard(session_id):
    """Main settings keyboard with NEW PDF features"""
    return [
        [
        ...
        [
            Button.inline("📄 PDF: Remove Pages", f"set_pdf_remove_{session_id}"),
            Button.inline("➕ PDF: Add Page", f"set_pdf_add_{session_id}")
        ],
        [
            Button.inline("🖼️ PDF: Thumbnail", f"set_pdf_thumb_{session_id}"),
            Button.inline("🎬 Video: Thumbnail", f"set_thumb_{session_id}")
        ],
        [
...
def get_confirm_keyboard(session_id, settings):
    ...
    if settings.get('pdf_pages'):
        settings_text += f"📄 PDF Remove Pages:\n`{settings['pdf_pages']}`\n\n"
    
    if settings.get('pdf_keywords'):
        settings_text += f"🔍 PDF Remove Keywords:\n`{', '.join(settings['pdf_keywords'][:3])}`\n\n"
    
    if settings.get('pdf_reference_image'):
        settings_text += f"📸 PDF Remove by Image: ✅\n\n"
    
    # PDF Page Addition (NEW)
    if settings.get('pdf_add_image'):
        pos = settings.get('pdf_add_position', 'end')
        settings_text += f"➕ PDF Add Page:\nPosition: `{pos}` | Image: ✅\n\n"
    
    # PDF Thumbnail (NEW)
    pdf_thumb = settings.get('pdf_thumbnail_action', 'keep')
    if pdf_thumb == 'remove':
        settings_text += f"🖼️ PDF Thumbnail: Remove\n\n"
    elif pdf_thumb == 'custom' and settings.get('pdf_custom_thumbnail'):
        settings_text += f"🖼️ PDF Thumbnail: Custom ✅\n\n"
    
    # Video Thumbnail
    thumb_mode = settings.get('thumbnail_mode', 'original')
    if thumb_mode == 'generate':
        settings_text += f"🎬 Video Thumbnail: Generate\n\n"
    elif thumb_mode == 'smart':
        skip = settings.get('thumbnail_skip', 10)
        settings_text += f"🎯 Video Thumbnail: Smart (skip {skip}s)\n\n"
    
    if not any([settings.get('find_name'), settings.get('find_cap'), settings.get('extra_cap'), 
                settings.get('pdf_pages'), settings.get('pdf_add_image'), 
                settings.get('pdf_thumbnail_action') != 'keep', 
                settings.get('thumbnail_mode') != 'original']):
...
def get_pdf_remove_options_keyboard(session_id):
    """PDF page REMOVAL options"""
    return [
        ...
        [Button.inline("⏭️ Skip Remove Settings", f"skip_{session_id}")],
        [Button.inline("❌ Cancel", f"cancel_{session_id}")]
    ]

def get_pdf_add_options_keyboard(session_id):
    """🆕 PDF page ADDITION options"""
    return [
        [Button.inline("📤 Add at Start", f"pdf_add_start_{session_id}")],
        [Button.inline("📥 Add at End", f"pdf_add_end_{session_id}")],
        [Button.inline("📍 Add at Custom Position", f"pdf_add_custom_{session_id}")],
        [Button.inline("⏭️ Skip Add Settings", f"skip_{session_id}")],
        [Button.inline("❌ Cancel", f"cancel_{session_id}")]
    ]

def get_pdf_thumbnail_options_keyboard(session_id):
    """🆕 PDF thumbnail options"""
    return [
        [Button.inline("📌 Keep Original", f"pdf_thumb_keep_{session_id}")],
        [Button.inline("🗑️ Remove Thumbnail", f"pdf_thumb_remove_{session_id}")],
        [Button.inline("🖼️ Add Custom Thumbnail", f"pdf_thumb_custom_{session_id}")],
